...
{
	char*	name; // 命令名称
	int(*handler)(int, char**); // shell_command 函数指针
	char	conditions; // 命令执行的条件
} COMMAND;
// 函数的声明
// extern文件 在ext2_shell.c中定义
...
void do_shell(void)
{
	...
	printf("%s File system shell (designed by zicheng)\n", g_fs.name);

	while (-1)
	{
		printf("OS-zicheng: [/%s]$ ", g_currentDir.name);
		fgets(buf, 1000, stdin); // 从标准输入读command到缓冲区
	
		argc = seperate_string(buf, argv); // 拆分命令
		if (argc == 0)
			continue;
		// 匹配命令
